...
export async function getTopRatings(page: number, tab: TimeControl, country: string, search: string, sort: SortBy, dir: SortDirection, onlyActive: string):Promise<TopRatingsResponse> {
  try {
        if(page === 0 && country === "ALL" && search === "" && sort === SortBy.RATING && dir === SortDirection.DESC && onlyActive === "true") {
        ...
        console.log(`http://193.122.162.246:8080/${endpoint}?page=${page}&country=${country}&search=${search}&sort=${sort}&dir=${dir}&onlyActive=${onlyActive}`)
        const response = await fetch(`http://193.122.162.246:8080/${endpoint}?page=${page}&country=${country}&search=${search}&sort=${sort}&dir=${dir}&onlyActive=${onlyActive}`);
